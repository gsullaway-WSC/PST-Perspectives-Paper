...
prepare_data <- function(df) {
  ...
  df_clean <- df %>%
    ...
    mutate(
      ...
      release_location_rmis_region = factor(release_location_rmis_region)
...
#    group_by(hatchery_name) %>%
    ...
    mutate(
      ...
      release_location_rmis_region = fct_drop(release_location_rmis_region)
...
fit_lme_by_age <- function(df, ocean_age_value, include_sex = FALSE) {
  ...
  if(ocean_age_value == 5) { # only one level of FW age for age 5 fish so removing from model 
    fixed_formula <- "length ~ year + fishery + run_type + day_of_year + release_stage + release_location_rmis_region"#hatchery_name"
} else {
  fixed_formula <- "length ~ year + fishery + freshwater_age + run_type + day_of_year + release_stage + release_location_rmis_region"
   }
  ...
  m2 <- tryCatch({
    lme(as.formula(fixed_formula),
        random = ~1|release_location_rmis_region, #hatchery_name
        data = df_age,
        method = "REML",
        na.action = na.omit)
  }, error = function(e) NULL)
  
  # Model 4: Year nested within release_location_rmis_region hatchery_name (as in Ohlberger)
  # m3 <- tryCatch({
  #   lme(as.formula(fixed_formula),
  ...
  m_var_pop <- tryCatch({
    update(best_model, weights = varIdent(form = ~1|release_location_rmis_region))#hatchery_name))
  }, error = function(e) NULL)
...
# CALL and RUN ===========
## load data all PS ====
df1 <- read_csv("data/PS_Chinook_RMIS_tidy.csv") %>% 
...
# once they work individually, run them all in one so its easier for plotting etc. 
 ...
 write_rds(results, "output/PS_all_sizeatage_LME_results.RDS")
  
 ## PS_N ====
 df2 <- read_csv("data/PS_Chinook_RMIS_tidy.csv") %>% 
   filter(!brood_year < 1980,
          release_location_rmis_region %in% c("NPS", "SKAG", "NOWA", "JUAN")) 
 
 # once they work individually, run them all in one so its easier for plotting etc. 
 results <- run_full_analysis(input_data = df2, ocean_ages = 2:4)
 
 write_rds(results, "output/PS_N_sizeatage_LME_results.RDS")
 
 ## PS_S ====
 df3 <- read_csv("data/PS_Chinook_RMIS_tidy.csv") %>% 
   filter(!brood_year < 1980#,
          #run == "Fall"
   ) %>%
   filter(release_location_rmis_region %in% c("SPS", "HOOD","MPS"))#, "CECR"))
 
 # once they work individually, run them all in one so its easier for plotting etc. 
 results <- run_full_analysis(input_data = df3, ocean_ages = 2:4)
 
 write_rds(results, "output/PS_S_sizeatage_LME_results.RDS")
