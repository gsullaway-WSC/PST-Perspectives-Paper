...
# CALL and RUN ===========
## load data ====
df <- read_csv("data/OR_Coast_Chinook_RMIS_tidy.csv") %>% 
  filter(!brood_year < 1980#,
         #run == "Fall"
         ) %>%
  filter(release_location_rmis_region %in% c("SOOR", "NOOR","LOCR"))#, "CECR"))

t <- data.frame(unique(df[c("release_location_rmis_region","release_location_rmis_basin", "hatchery_location_name")]))

#unique(df$release_location_rmis_region)
## call functions for all ages ====
 
# all_results <-list(results1,results1,results1,results1)
# results <- run_full_analysis(input_data = df, ocean_ages = 2)
...
# once they work individually, run them all in one so its easier for plotting etc. 
 ...
 write_rds(results, "output/OR_Coast_sizeatage_LME_results.RDS")
