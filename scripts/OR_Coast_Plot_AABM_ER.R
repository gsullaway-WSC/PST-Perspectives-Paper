...
esc_goals <- read_csv("data/Escapement_goals_data_all_2026-02-19.csv") %>%
  filter(SeriesLabel %in% c("PSC-Agreed Goal","Agency Goal","PSC Goal",
                            ...
                            "Pre-PSC Goal: MR Esc"),
         StockName %in% c("Nehalem R.", "Siletz Fall","Siuslaw R.", 
                          "South_umpqua", "Coquille")) %>%          
  rename(year = "Year") %>%
  ...
  dplyr::mutate(population = case_when(StockName == "Siuslaw R."  ~ "Siuslaw Fall",
                         ...
                         TRUE ~ StockName),
                critical_esc = case_when(population == "Nehalem Fall" ~ 5369,
                                         population == "Siletz Fall" ~ 3471,
...
years_seq <- 1986:2024  # adjust end year as needed

new_pops <- bind_rows(
  data.frame(
    year        = years_seq,
    StockName   = "South_umpqua",
    SeriesLabel = "State Esc Threshold",
    Values      = NA,        # replace with correct goal if different from critical
    population  = "South Umpqua",
    critical_esc = 1493,
    desired_esc = 8284
  ),
  data.frame(
...
esc_goals <- bind_rows(esc_goals, new_pops)
  ...
  dplyr::mutate(esc_tot = as.numeric(esc_tot),
               ...
               aabm_tot = as.numeric(aabm_tot), 
                under_goal = case_when(esc_tot < esc_goal ~ "Under Esc. Goal",
                                       TRUE ~ "NA")) %>% 
  group_by(population) %>%
  mutate(desired_esc = desired_esc,
...
esc_goal_plot <- ggplot(joined_df, aes(x = year, y = esc_tot)) +
  ...
  geom_path(aes(y=desired_esc), color = "darkblue", linetype =2) + 
  ...
  labs(
    title = "Oregon Coast Chinook Escapement vs. Goal",
    subtitle = "Red shading = below critical escapement goal & above-average AABM catch (vulnerable years)\nBlack dashed line = state critical escapement\nBlue dashed line = state desired escapement",
    x = "Year", y = "Escapement"
  ) + 
...
ggsave("output/plots/OR_Coast_esc_goal_vulnerable.jpeg",
       plot = esc_goal_plot,
       width = 9, height = 5,
       dpi = 300, units = "in")
