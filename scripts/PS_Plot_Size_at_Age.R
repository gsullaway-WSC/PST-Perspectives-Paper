...
# Predicted lengths Stacked ALL PS ====== 
results <- readRDS("output/PS_all_sizeatage_LME_results.RDS")
...
predicted_plot_together <- ggplot(combined_effects_in %>% filter(!ocean_age %in% c(1,5),!year ==1980), 
  ...
  scale_fill_manual(values = c("1" = "#E69F00", "2" = "#56B4E9", 
                    ...
                    name = "Ocean Age") +
  labs(
    title = "Trends in Puget Sound Chinook Salmon\nPredicted Length-at-Age",
    x = "Brood Year",
...
ggsave("output/plots/Puget Sound Plots/PS_all_size_at_age_predicted_together.png", predicted_plot_together, 
       width = 8, height = 9, dpi = 300)

# Predicted lengths Stacked North PS ====== 
results <- readRDS("output/PS_N_sizeatage_LME_results.RDS")
...
# First, complete the data with explicit NAs for missing years
combined_effects_complete <- combined_effects_in %>%
  filter(!ocean_age %in% c(1,5), !year == 1980, !year == 2020, !year == 2014, !year == 2015) %>%
  group_by(ocean_age) %>%
  complete(year = full_seq(year, 1)) %>%  # fills in missing years with NA
  ungroup()

predicted_plot_together <- ggplot(combined_effects_complete, 
                                  aes(x = year, y = predicted_length, 
                                      color = factor(ocean_age), 
                                      fill = factor(ocean_age))) +
  geom_ribbon(aes(ymin = predicted_lower, ymax = predicted_upper), 
              alpha = 0.2, color = NA, na.rm = TRUE) +
  geom_line(size = 1.2) +   # na.rm = FALSE (default) so NAs break the line
  ...
  scale_fill_manual(values = c("1" = "#E69F00", "2" = "#56B4E9", 
                    ...
                    name = "Ocean Age") +
  labs(
    title = "Trends in North Puget Sound Chinook Salmon\nPredicted Length-at-Age",
    x = "Brood Year",
    y = "Predicted Length (in)"
  ) +
  theme_bw(base_size = 14) +
...
ggsave("output/plots/Puget Sound Plots/PS_N_size_at_age_predicted_together.png", predicted_plot_together, 
       width = 8, height = 9, dpi = 300)

# Predicted lengths Stacked South PS ====== 
results <- readRDS("output/PS_S_sizeatage_LME_results.RDS")
...
predicted_plot_together <- ggplot(combined_effects_in %>% filter(!ocean_age %in% c(1,5),!year ==1980), 
  ...
  scale_fill_manual(values = c("1" = "#E69F00", "2" = "#56B4E9", 
                    ...
                    name = "Ocean Age") +
  labs(
    title = "Trends in South Puget Sound Chinook Salmon\nPredicted Length-at-Age",
    x = "Brood Year",
    y = "Predicted Length (in)"
  ) +
  theme_bw(base_size = 14) +
...
ggsave("output/plots/Puget Sound Plots/PS_S_size_at_age_predicted_together.png", predicted_plot_together, 
       width = 8, height = 9, dpi = 300)
...
ggsave("output/plots/PS_all_size_at_age_predicted_together.png", predicted_plot_together, 
       width = 8, height = 9, dpi = 300)
