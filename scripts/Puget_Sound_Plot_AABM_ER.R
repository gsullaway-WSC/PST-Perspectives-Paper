...
PS_total_run_df <- data %>%  
  ...
  filter(region %in% c("PS"),
         population %in% c("Nooksack", "Stillaguamish", "Skagit")) %>% 
...
joined_df <- PS_total_run_df %>%
  ...
  # rename(esc_goal = "Agency Goal Natural Origin") %>%
...
ggsave("output/plots/PugetSound_esc_goal_plotER.jpeg",
...
ggsave("output/plots/PugetSound_esc_goal_ER_plot.jpeg",
       plot = new_ER_Plot,
       width = 8, height = 5,
       dpi = 300, units = "in")
...
ggsave("output/plots/Stillaguamish_esc_goal_ER_plot.jpeg",
       plot = new_ER_Plot,
       width = 8, height = 5,
       dpi = 300, units = "in")
...
new_ER_Plot<-ggplot(joined_df %>% 
  ...
  labs(
    title    = "Skagit Chinook Salmon Escapement vs. Exploitation Rates",
    subtitle = "Red shading = Exploitation Rates\n No Escapement Goals",
    x = "Year", y = "Escapement"
  ) +
...
ggsave("output/plots/Skagit_esc_goal_ER_plot.jpeg",
       plot = new_ER_Plot,
       width = 8, height = 5,
       dpi = 300, units = "in")
